...
def get_news_digest(teams=None, players=None):
    ...
    try:
        ...
        if not api_key:
            logger.warning("No GOOGLE_API_KEY found in environment variables, returning fallback response")
            return {
                'success': True,
                'digests': [
                    {
                        'type': 'team',
                        'subject': team,
                        'content': f'# {team}\n\n- Please configure GOOGLE_API_KEY in your environment variables to get live team updates.\n- You can find instructions in the README.md file.',
                        'sources': 'Generated using fallback content'
                    } for team in (teams or [])[:3]
                ] + [
                    {
                        'type': 'player',
                        'subject': player,
                        'content': f'# {player}\n\n- Please configure GOOGLE_API_KEY in your environment variables to get live player updates.\n- You can find instructions in the README.md file.',
                        'sources': 'Generated using fallback content'
                    } for player in (players or [])[:3]
                ],
        ...
        try:
            ...
            logger.error(f"Error creating Gemini client: {str(e)}")
            return {
                'success': False,
        ...
        if teams:
            ...
            for team in teams[:3]:  # Limit to 3 teams
                try:
                    ...
                    logger.error(f"Error generating content for team {team}: {str(e)}")
                    errors.append(f"Failed to generate content for {team}")
        ...
        if players:
            ...
            for player in players[:3]:  # Limit to 3 players
                try:
                    ...
                    logger.error(f"Error generating content for player {player}: {str(e)}")
                    errors.append(f"Failed to generate content for {player}")

        if not digests:
            if errors:
                return {
                    'success': False,
                    'error': 'Failed to generate content: ' + '; '.join(errors)
                }
            return {
                'success': False,
...
def get_search_content(response):
    ...
    try:
        ...
        logger.error(f"Error extracting search content: {str(e)}")
...
def clean_source_links(sources_html):
    ...
    try:
        ...
        logger.error(f"Error cleaning source links: {str(e)}")
        return [] 